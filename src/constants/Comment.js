export const ADD_COMMENT_START = 'ADD_COMMENT_START';
export const ADD_COMMENT_SUCCESS = 'ADD_COMMENT_SUCCESS';
export const ADD_COMMENT_FAIL = 'ADD_COMMENT_FAIL';


export const LOAD_COMMENTS_START = 'LOAD_COMMENTS_START';
export const LOAD_COMMENTS_SUCCESS = 'LOAD_COMMENTS_SUCCESS';
export const LOAD_COMMENTS_FAIL = 'LOAD_COMMENTS_FAIL';


export const UPDATE_COMMENT_START = 'UPDATE_COMMENT_START';
export const UPDATE_COMMENT_SUCCESS = 'UPDATE_COMMENT_SUCCESS';
export const UPDATE_COMMENT_FAIL = 'UPDATE_COMMENT_FAIL';



export const CLEAN_COMMENTS = 'CLEAN_COMMENTS';


export const LOAD_COMMENT_PERM_START = 'LOAD_COMMENT_PERM_START';
export const LOAD_COMMENT_PERM_SUCCESS = 'LOAD_COMMENT_PERM_SUCCESS';
export const LOAD_COMMENT_PERM_FAIL = 'LOAD_COMMENT_PERM_FAIL';